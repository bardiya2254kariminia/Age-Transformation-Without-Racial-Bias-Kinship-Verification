        ...
        for i in range(n_samples):
            ...
            # otherwise, create a new entry for the sample
                ...
                id_logs.append({f'input_age_{label}': float(input_ages[i]) * 100,
                                f'output_age_{label}': float(output_ages[i]) * 100,
                                f'target_age_{label}': float(target_ages[i]) * 100})
        loss = F.mse_loss(output_ages.unsqueeze(dim=1), target_ages)
        return loss, id_logs
...
if __name__ == "__main__":
    ...
    id_logs =[]
    y_hat = torch.zeros((1,3,256,256)).cuda()
    y = torch.zeros((1,3,256,256)).cuda()
    target_ages = torch.zeros((1,1)).cuda()
    aging_loss, id_logs = aging_loss(y_hat, y, target_ages, id_logs, label="real")