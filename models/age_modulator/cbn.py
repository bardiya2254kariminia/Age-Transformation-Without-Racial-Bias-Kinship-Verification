...
#         self.betas = nn.Parameter(torch.zeros(self.batch_size, self.channels).cpu())
#         self.gammas = nn.Parameter(torch.ones(self.batch_size, self.channels).cpu())
...
#         else:
#             delta_betas = torch.zeros(self.batch_size, self.channels).cpu()

#         if self.use_gammas:
#             delta_gammas = self.fc_gamma(lstm_emb)
#         else:
#             delta_gammas = torch.zeros(self.batch_size, self.channels).cpu()
#         # if self.use_betas:
#         #     delta_betas = self.fc_beta(lstm_emb)
#         # else:
...
#         # print(f"created_cbn_input : {delta_betas.shape=} , {delta_gammas.shape=}")
...
#         # print(f"{betas_cloned.shape=} , {delta_betas.shape=}")
#         # print(f"{gammas_cloned.shape=} , {delta_gammas.shape=}")
...
#         # print(f"{feature_normalized.shape=} , {gammas_expanded.shape =}")
...
class CBN(nn.Module):
    ...
    def create_cbn_input(self, lstm_emb):
        ...
        if self.use_betas:
            delta_betas = self.fc_beta(lstm_emb).cuda()
        else:
            delta_betas = torch.zeros(self.batch_size, self.channels).cuda()
